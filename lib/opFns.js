...
module.exports = {
  ...
  EXTCODESIZE: function (address, runState, cb) {
    var _address = address.slice(-20).toString("hex");
    while(_address.length < 40) { _address = "0"+_address; }
    var contract = runState.ds.expert.from({address: "0x"+_address}).contract;
    cb(null, utils.intToBuffer(contract.bytecode.length/2));
  },
  ...
  LOG: function (memOffset, memLength, s, runState, cb) {
    var args = Array.prototype.slice.call(arguments, 0)
    // var cb = args.pop() // pop off callback
    // var runState = args.pop()
    var topics = args.slice(2, args.length - 2)
    ...
    try{
    var logtr = runState.ds.expert.from({classId: utils.sha3(runState.code.toString("hex")).toString("hex")}).contract.logtr;
    } catch(e){
      console.log(e);
    }
    var translatedLog = logtr.translate({
      logIndex: 0,
    ...
    runState.ds.logStep(runState, translatedLog, (err, isSystemLog) => {
      if(!isSystemLog) {
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  runState.ds.iface.getBalance(runState, runState.ds.origin, (err, balance) => {
    if(err) return cb(err);

    if (runState.depth >= fees.stackLimit.v || balance.cmp(callOptions.value) === -1) {
      runState.stack.push(new Buffer([0]))
      cb()
    } else {
      // if creating a new contract then increment the nonce
      if (!callOptions.to) {
      ...
      co.value = '0x' + callOptions.value.toString('hex');
      if(callOptions.to) co.to = '0x'+callOptions.to.toString('hex');

      var isLibraryCall = 'to' in co && Object.keys(runState.ds.libs).indexOf(co.to) > -1;
      
      if(isLibraryCall) {
        runState.ds.iface.performLibraryCall(runState, co, localOpts, (err, gas, status) => {
          subGas(runState, new BN(gas));
          cb(err, status);
        });
      } else if(co.to &&
        co.to != '0x0000000000000000000000000000000000000000') {
          var contract = runState.ds.expert.from({address: co.to}).contract;
          // Test if the call is constant (call) or not (send transaction)
          var fabi;

          // getCode(co.to, (err, code) => {
            runState.ds.expert.from({
              address: co.to,
              function: co.data.slice(2,10),
              rtcode: contract.runtimeBytecode
            }).fabi;

            // Chain interaction can be one of the follwing form: 1. a static function call
            // 2. a local library call
            // 3. a chain interaction - function call, value transfear or deploy

            var isStaticCall = co.to && fabi && fabi.constant || runState.ds.callFlag;
            
            if(isStaticCall) { // 1. static function call
              runState.ds.iface.performStaticCall(runState, co, localOpts, cb);
            } else { // 3. transaction
              runState.ds.iface.performTransaction(runState, co, (err, gas, status) => {
                subGas(runState, new BN(gas));
                cb(err, status);
              });
            }
