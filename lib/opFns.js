...
module.exports = {
  ...
  LOG: function (memOffset, memLength) {
    ...
    // add data
    log.push(mem)
    // TODO - find an awesome way to distingulish Script contracts from others
    ...
    if(!runState.ds.logStep(runState, translatedLog )) {
      ...
      runState.ds.web3.eth.sendTransaction(co, (err, txHash) => {
        ...
        runState.ds.web3.eth.getTransactionReceipt(txHash, (err, res) => {
          if( res.contractAddress ) {
            ...
            runState.ds.addressclassMap[res.contractAddress] = className;
            console.log(`NEW   new ${className}(${params})`);
            cb(err, new Buffer(res.contractAddress.slice(2), 'hex'))
          } else {
            var className = runState.ds.addressclassMap[co.to];
...
function staticCall(runState, co) {
  return co.to &&
       co.data &&
       co.to in runState.ds.addressclassMap &&
       co.data.slice(2,10) in runState.ds.classfunctionconstantsMap[runState.ds.addressclassMap[co.to]] &&
       runState.ds.classfunctionconstantsMap[runState.ds.addressclassMap[co.to]][co.data.slice(2,10)];
}
