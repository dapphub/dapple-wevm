...
module.exports = {
  ...
  LOG: function (memOffset, memLength) {
    ...
    console.log(runState.ds.logtranslator.translate({
      logIndex: 0,
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  if (runState.depth >= fees.stackLimit.v || new BN(runState.contract.balance).cmp(callOptions.value) === -1) {
    ...
    if(staticCall(runState, co)) {
      runState.web3.eth.call(co, (err, result) => {
      ...
      var sourceHash = utils.sha3(co.data.slice(2)).toString('hex');
      runState.web3.eth.sendTransaction(co, (err, txHash) => {
        if (err || !txHash) cb(err, new Buffer([0]));
        runState.web3.eth.getTransactionReceipt(txHash, (err, res) => {
          if( res.contractAddress ) {
            if( sourceHash in runState.sourceclassMap ) {
              runState.addressclassMap[res.contractAddress] = runState.sourceclassMap[sourceHash];
              console.log(`DEPLOYING Contract "${runState.sourceclassMap[sourceHash]}"`);
            }
