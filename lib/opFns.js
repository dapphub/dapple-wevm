...
module.exports = {
  ...
  EXTCODESIZE: function (address, runState, cb) {
    var getCode = runState.ds.web3.eth.getCode;
    var _address = address.slice(-20).toString("hex");
    while(_address.length < 40) { _address = "0"+_address; }
    getCode('0x'+_address, (err, code) => {
      cb(err, (code.length - 2).toString(16));
    });
  },
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  // TODO - export web3 to web3Interface
  ...
  runState.ds.iface.getBalance(runState, runState.ds.origin, (err, balance) => {
    ...
    if (runState.depth >= fees.stackLimit.v || balance.cmp(callOptions.value) === -1) {
      ...
      // Chain interaction can be one of the follwing form:
      // 1. a static function call
      ...
      } else if(isLibraryCall) { // 2. local library call
