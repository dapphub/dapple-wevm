...
module.exports = {
  ...
  LOG: function (memOffset, memLength) {
    ...
    var memOffset_ = memOffset;
    var memLength_ = memLength;
    ...
    // add data
    log.push(mem)
    // TODO - THIS IS HOW FUCKING REASONING ABOUT LOGS
    var translatedLog = runState.ds.logtranslator.translate({
      logIndex: 0,
      transactionIndex: "0x0",
      address: "0x0",
      data: '0x'+mem.toString('hex'),
      topics: topics.map(t => '0x'+t.toString('hex')),
      type: "mined"
    });
    switch (translatedLog.event) {
      case 'setCalls':
        // TODO - beware of throws!!
        runState.ds.callFlag = translatedLog.args.flag;
        break;
      case 'setOrigin':
        // TODO - check if user owns account
        runState.ds.origin = translatedLog.args.origin;
        console.log(`ACC   switch origin to ${translatedLog.args.origin}`);
        break;
      default:
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  // TODO - simulate call
  // TODO - implement gas knowledge base for each call
  ...
  if (runState.depth >= fees.stackLimit.v || new BN(runState.contract.balance).cmp(callOptions.value) === -1) {
    ...
    var co = {};
    // co.from = '0x'+callOptions.origin.toString('hex');
    co.from = runState.ds.origin;
    co.data = '0x'+callOptions.data.toString('hex');
    if(callOptions.to) co.to = '0x'+callOptions.to.toString('hex');
    // Test if the call is constant (call) or not (send transaction)
    if(staticCall(runState, co) || runState.ds.callFlag) {
    ...
    } else if('to' in co && co.to === '0x0000000000000000000000000000000000000000') {
      var functionAbi = runState.ds.class_x_function_to_object["SH"][co.data.slice(2,10)];
      ...
      runState.ds.web3.eth.sendTransaction(co, (err, txHash) => {
        if (err || !txHash) cb(err, new Buffer([0]));
        runState.ds.web3.eth.getTransactionReceipt(txHash, (err, res) => {
          if( res.contractAddress ) {

            var className = "";
            var constructorData = "";
            Object.keys(runState.ds.sourceclassMap2).forEach(code => {
              if(co.data.indexOf(code) === 2) {
                className = runState.ds.sourceclassMap2[code];
                constructorData = co.data.slice(code.length - co.data.length + 2);
              }
            });
            var types = runState.ds.class_x_function_to_object[className]['constructor'].inputs.map(i => i.type)
            var params = runState.ds.coder.decodeParams(types, constructorData).join(', ');

            runState.addressclassMap[res.contractAddress] = className;
            console.log(`NEW   new ${className}(${params})`);
            cb(err, new Buffer(res.contractAddress.slice(2), 'hex'))
          } else {
            var className = runState.addressclassMap[co.to];

            var functionAbi = runState.ds.class_x_function_to_object[className][co.data.slice(2,10)];
            var params = runState.ds.coder.decodeParams(functionAbi.inputs.map(i => i.type), co.data.slice(10)).map(p => p.toString()).join(', ');
            console.log(`TXR   ${className}(${co.to}).${functionAbi.name}(${params})`);
            cb(err, new Buffer([1]))
          }
        })
      });
...
// Decides whether to call a function or send a transaction
//
// if(!!co.to) {
//   console.log('...');
//   console.log(co.to);
//   console.log(co.data);
//   console.log(runState.addressclassMap[co.to]);
//   console.log(co.data.slice(2,10), runState.ds.classfunctionconstantsMap[runState.addressclassMap[co.to]][co.data.slice(2,10)]);
//   console.log(co.data.slice(2,10) in runState.constantsMap);
//   console.log(runState.constantsMap[co.data.slice(2,10)]);
//   console.log('...');
// }
// 
// TODO - add address knowledgebase
function staticCall(runState, co) {
  return co.to &&
       co.data &&
       co.to in runState.addressclassMap &&
       co.data.slice(2,10) in runState.ds.classfunctionconstantsMap[runState.addressclassMap[co.to]] &&
       runState.ds.classfunctionconstantsMap[runState.addressclassMap[co.to]][co.data.slice(2,10)];
}
