...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  var balance = runState.ds.iface.getBalance(runState, runState.ds.origin);
  if (runState.depth >= fees.stackLimit.v || balance.cmp(callOptions.value) === -1) {
    ...
    var isLibraryCall = false && 'to' in co && co.to === '0x0000000000000000000000000000000000000000';
    // library calls currently disabled
    ...
    if(isStaticCall) { // 1. static function call
      ...
      // TODO - export this to modular addon architecture
      runState.ds.iface.performLibraryCall(runState, co, localOpts, cb );
    } else { // 3. transaction
